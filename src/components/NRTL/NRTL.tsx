...
// hooks | library
import { useState, useMemo, useEffect, ReactElement, ChangeEvent, CSSProperties } from 'react'
...
export default function NRTL({
	...
	const [filters, setFilters] = useState<string[]>(datas.tableHead.map((): string => ''))

	const removeAccents: (string: string) => string = (string: string): string => {
		return string.normalize('NFD').replace(/[\u0300-\u036f]/g, '')
	}

	const sortedData: string[][] | undefined = useMemo((): string[][] | undefined => {
		let filteredData: string[][] = [...datas.tableBody]

		filters.forEach((filter: string, columnIndex: number): void => {
			if (filter) {
				filteredData = filteredData.filter((row: string[]): boolean => row[columnIndex] === filter)
			}
		})
		...
		if (searchTerm.length > 0) {
			filteredData = filteredData.filter((row: string[]): boolean => {
				return row.some((cell: string): boolean => {
					return removeAccents(cell.toLowerCase()).includes(removeAccents(searchTerm.toLowerCase()))
				})
			})
		}
	...
	const getColumnOptions: (columnIndex: number) => string[] = (columnIndex: number): string[] => {
		const uniqueOptions = new Set(datas.tableBody.map((row: string[]): string => row[columnIndex]))
		return Array.from(uniqueOptions).sort((a: string, b: string): number => a.localeCompare(b))
	}
			...
			<div className={'tableHeader'}>
				{showItemsPerPageSelector && itemsPerPageOptions.length > 0 && (
					<div className='selectContainer'>
						<label htmlFor='itemsPerPage'>{language === 'En' ? 'Show' : 'Afficher'}</label>
						...
						<label htmlFor='itemsPerPage'>{language === 'En' ? 'entries' : 'entrées'}</label>
					</div>
				)}
				{showSearchBar && (
					<div className='searchContainer'>
						<label htmlFor='filter'>{language === 'En' ? 'Search' : 'Rechercher'}: </label>
				...
				<table>
							...
							{datas.tableHead.map(
								(head: string, index: number): ReactElement => (
									...
									>
										{head}
										{enableColumnSorting && (
											<div onClick={enableColumnSorting ? (): void => requestSort(index) : undefined}>
												<span
													className={`chevron ${
														sortConfig?.key === index && sortConfig.direction === 'ascending' ? 'chevron-active' : ''
													}`}
												>
												...
												<span
													className={`chevron ${
														sortConfig?.key === index && sortConfig.direction === 'descending' ? 'chevron-active' : ''
													}`}
												>
													<svg width='12' height='12' viewBox='0 0 20 20'>
										...
										{filterableColumns && filterableColumns[index] && (
											<div className={'selectContainer'}>
												<select
													onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFilterChange(index, e.target.value)}
													defaultValue={''}
												>
					...
					<tbody>
						{currentData.length > 0 ? (
							...
							<tr>
								<td colSpan={datas.tableHead.length} style={{ textAlign: 'center' }}>
					...
					{showPreviousNextButtons && (
						<div className='buttonContainer'>
							{screenSize.width < 768 ? (
								<>
									<IoChevronBack className={page === 1 ? 'disabled' : 'paginationIcn'} onClick={handlePreviousPage} />
									<IoChevronForward
								...
								<>
									...
									<button className='button' onClick={handleNextPage} disabled={page === totalPages}>
