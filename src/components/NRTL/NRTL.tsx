...
interface INRTLProps {
  ...
  onServerSearchChange?: (searchTerm: string) => void;
}
interface INRTL {
  tableHead: string[];
  tableBody: string[][];
...
export default function NRTL({
  ...
  const localTotalPages: number = Math.ceil(sortedData!.length / itemsPerPage);
  const serverTotalPages: number = Math.ceil(
    serverTotalItems / serverItemsPerPage,
  );
  ...
  }, [sortedData, page, showPagination, itemsPerPage, serverSidePagination]);
  ...
  const handleNextPage: () => void = (): void => {
    setPage((prevPage: number): number =>
      Math.min(prevPage + 1, localTotalPages),
    );
  };
        ...
        {showItemsPerPageSelector && itemsPerPageOptions.length > 0 && (
          <div className="selectContainer">
            ...
            {serverSidePagination ? (
              <select
                id={"itemsPerPage"}
                value={serverItemsPerPage}
                onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                  onServerItemsPerPageChange?.(Number(e.target.value))
                }
              >
          ...
          {((): ReactElement => {
            if (serverSidePagination) {
              return (
                <p>
                  {language === "En"
                    ? `Showing ${currentData.length} of ${serverTotalItems} entries`
                    : ` Affichage de ${currentData.length} sur ${serverTotalItems} entrées`}
                </p>
              );
            } else {
              return (
            ...
            <div className="buttonContainer">
              {screenSize.width < 768 ? (
                <>
                  ...
                  <IoChevronForward
                    className={
                      page === localTotalPages ? "disabled" : "paginationIcn"
                    }
